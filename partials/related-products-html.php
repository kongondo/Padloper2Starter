        ...
        if ($relatedProducts->count()) {
            foreach ($relatedProducts as $product) {
                ...
                <form method='post' class='padloper-cart-add-product action='{$config->urls->root}padloper/add/'>" .
                    ...
                    "<div class='flex items-end justify-end h-56 w-full bg-cover' style='background-image: url({$thumb->url})'>" .
                    // related product add to cart button
                    "<button
                        class='p-2 rounded-full bg-indigo-500 text-white mx-5 -mb-4 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-600' hx-post='{$post}' hx-target='#cart-summary' hx-swap='none'
                        >
